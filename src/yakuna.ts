...
export const ReplaceCase = /[\u200B-\u200D\uFEFF\u3164\s]|(\<br\s*\/?>\s*)+|(\<\/?p>\s*)+/g;
...
export class Yakuna {
  ...
  watch() {
    ...
    user.on('message', (msg: Response) =>
      this.handleMessage(new Response(msg))
    );
  ...
  private handleMessage(msg: Response) {
    if (msg.isInflammable()) {
      ...
      if (
        あひる焼き.test(msg.content().replace(ReplaceCase, '')) &&
        !this.criminals.has(msg.id())
      ) {
        this.yakuna(msg);
      }
    }
