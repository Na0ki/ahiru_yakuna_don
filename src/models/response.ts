...
export class Response {
  ...
  isInflammable(): boolean {
    return (
      this.event === 'update' ||
      (this.event === 'notification' && this.data.type === 'mention')
    );
  }
  ...
  username(): string {
    const url =
      ...
      (this.data.status &&
        this.data.status.account &&
        this.data.status.account.url);
    const [, hostname, username] =
      url.match(/https:\/\/(.+)\/(@.+)/) || new Array(3);
  ...
  content() {
    ...
    const content =
      this.data.content || (this.data.status && this.data.status.content) || '';
